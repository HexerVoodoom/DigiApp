...
| Sistema | Regra |
|---|---|
| ❤️ Corações (HP) | Perde na virada do dia: `floor((1 − feitas/meta) × maxHP)`, onde meta = `min(cadastradas, requisito do estágio)`. Cumpriu o requisito (ou fez tudo que cadastrou) = não perde. HP aceita frações de 0.5. HP 0 → degeneração (salvo pelo 💚 Coração Verde, ver linha abaixo). |
| 💚 Coração Verde | Guardrail anti-degeneração (`guardianHeartCharge`, 0–5, `GUARDIAN_HEART_CHARGE_NEEDED` em `types/progression.ts`). Carrega **+1 por dia perfeito** (independente do contador de evolução `perfectDays`) e **NÃO decai** em dia ruim — não é streak, é cura lenta. Ao chegar em 5/5 e o HP zerar na virada do dia (`useDailyReset.ts`), o escudo **absorve o golpe**: a degeneração NÃO acontece, o pet sobrevive com **1 HP**, e o carregador zera (precisa de mais 5 dias perfeitos pra recarregar). Sem o escudo carregado, degeneração ocorre normalmente. Badge 💚 no HUD (ao lado dos corações) mostra a carga; `DailyReportModal` avisa quando o escudo é usado. |
